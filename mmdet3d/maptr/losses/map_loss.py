...
def diff_weighted_loss(loss_func):
    """Create a weighted version of a given loss function.
    ...
    def wrapper(pred,
                ...
                **kwargs):
        # get element-wise loss
        loss = loss_func(pred, target, labels, **kwargs)
...
def diff_pts_l1_loss(pred, target, labels, weight, reduction, avg_factor):
    instance_num, pts_num, _ = pred.size()
    if target.numel() == 0:
        return pred.sum() * 0
    assert pred.size() == target.size()
    pred_tmp = pred.detach()
    vector_dic_v1 = (pred_tmp[:, :-1, :] - pred_tmp[:, 1:, :]) + 1e-9  # 各点之间的矢量（第一点不取）  num_instace, num_pts - 1, 2 (0-1, 1-2)
    vector_dic_v2 = (target[:, :-1, :] - target[:, 1:, :]) + 1e-9

    weight_vector_gt_v1 = vector_dic_v2[:, 1:, :]  # num_instace, num_pts - 2, 2
    weight_vector_gt_v2 = vector_dic_v2[:, :-1, :]

    dot_product = torch.sum(weight_vector_gt_v1 * weight_vector_gt_v2, dim=2, keepdim=True)
    magnitude_v1 = torch.norm(weight_vector_gt_v1, dim=2, keepdim=True)
    magnitude_v2 = torch.norm(weight_vector_gt_v2, dim=2, keepdim=True)
    weight_dir = dot_product / (magnitude_v1 * magnitude_v2)
    weight_dir = torch.exp((1.0 - weight_dir) / 2)

    dot_product = torch.sum(vector_dic_v1 * vector_dic_v2, dim=2, keepdim=True)  # num_instace, num_pts, 2
    magnitude_v1 = torch.norm(vector_dic_v1, dim=2, keepdim=True)
    magnitude_v2 = torch.norm(vector_dic_v2, dim=2, keepdim=True)
    cos_angle = dot_product / (magnitude_v1 * magnitude_v2)

    loss = torch.cat([torch.zeros(instance_num, 1, 1).cuda(), ((1.0 - cos_angle) / 2)[:, :-1, :] * weight_dir, torch.zeros(instance_num, 1, 1).cuda()], dim=1)
    if weight is not None:
        loss = loss * weight
...
def diff_pts_l1_loss(pred, target, labels, weight, reduction, avg_factor):
    ...
    loss = torch.cat([torch.zeros(instance_num, 1, 1).cuda(), (1.0 - cos_angle / 2)[:, :-1, :]], dim=1)
    if weight is not None:
        loss = loss * weight
...
class DiffPtsL1Loss(nn.Module):
    ...
    def __init__(self, reduction='mean', loss_weight=1.0):
        super(DiffPtsL1Loss, self).__init__()
        self.reduction = reduction
        self.loss_weight = loss_weight
    ...
    def forward(self,
        ...
        reduction = (
            reduction_override if reduction_override else self.reduction)
        # import pdb;pdb.set_trace()
        loss_bbox = self.loss_weight * diff_pts_l1_loss(
            pred, target, labels, weight, reduction=reduction, avg_factor=avg_factor)
        return loss_bbox
