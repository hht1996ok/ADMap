...
class MapTRHead(DETRHead):
    ...
    def _init_layers(self):
        ...
        if self.use_fast_perception_neck:
            num_pred = (self.transformer.decoder[0].num_layers + 1) if \
                self.as_two_stage else self.transformer.decoder[0].num_layers
        else:
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None, only_bev=False):
        ...
        if self.use_fast_perception_neck:
            bev_embed_list, depth, hs_list, init_reference_list, inter_references_list = outputs
    ...
    def loss_single(self,
        ...
        loss_diff_pts = self.loss_diff_pts(
            pts_preds[isnotnan,:,:], normalized_pts_targets[isnotnan, :,:], labels[isnotnan],
        ...
        return loss_cls, loss_bbox, loss_iou, loss_pts, loss_dir, loss_diff_pts
    ...
    def loss(self,
        ...
        if self.use_fast_perception_neck:
            ...
            for head_id in range(len(preds_dicts['all_cls_scores'])):
                ...
                # import pdb;pdb.set_trace()
                losses_cls, losses_bbox, losses_iou, losses_pts, losses_dir, losses_diff_pts = multi_apply(
                ...
                loss_dict[f'{head_id}_loss_bbox'] = losses_bbox[-1] * head_weights[head_id]
                loss_dict[f'{head_id}_loss_iou'] = losses_iou[-1] * head_weights[head_id]
                ...
                for loss_cls_i, loss_bbox_i, loss_iou_i, loss_pts_i, loss_dir_i, loss_diff_i in zip(losses_cls[:-1],
                    ...
                    loss_dict[f'{head_id}_d{num_dec_layer}.loss_bbox'] = loss_bbox_i * head_weights[head_id]
                    loss_dict[f'{head_id}_d{num_dec_layer}.loss_iou'] = loss_iou_i * head_weights[head_id]
            ...
            if enc_cls_scores is not None:
                ...
                enc_loss_cls, enc_losses_bbox, enc_losses_iou, enc_losses_pts, enc_losses_dir = \
                    self.loss_single(enc_cls_scores, enc_bbox_preds, enc_pts_preds,
                                    gt_bboxes_list, binary_labels_list, gt_pts_list,gt_bboxes_ignore, img_metas)
                loss_dict['enc_loss_cls'] = enc_loss_cls
                loss_dict['enc_loss_bbox'] = enc_losses_bbox
                loss_dict['enc_losses_iou'] = enc_losses_iou
                loss_dict['enc_losses_pts'] = enc_losses_pts
