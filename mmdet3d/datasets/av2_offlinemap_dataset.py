    ...
    def shift_fixed_num_sampled_points_v2(self):
        ...
        for idx, instance in enumerate(self.instance_list):
            ...
            multi_shifts_pts_tensor[:, :, 1] = torch.clamp(multi_shifts_pts_tensor[:, :, 1], min=-self.max_y,
                                                           max=self.max_y)
            multi_shifts_pts_tensor[:, :, 2] = torch.clamp(multi_shifts_pts_tensor[:, :, 2], min=self.min_z,
                                                           max=self.max_z)
...
class VectorizedAV2LocalMap(object):
    ...
    def gen_vectorized_samples(self, map_annotation, example=None, feat_down_sample=32):
        ...
        gt_instance = LiDARInstanceLines(gt_instance, gt_labels, self.sample_dist,
                                         ...
                                         patch_size=self.patch_size, code_size=self.code_size)
...
class CustomAV2OfflineLocalMapDataset(CustomNuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        lidar_path = '/dahuafs/groupdata/Lidaralgorithm/' + info['lidar_path']
        input_dict = dict(
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                img_fpath = '/dahuafs/groupdata/Lidaralgorithm/' + cam_info['img_fpath']
                image_paths.append(img_fpath)
                # camera intrinsics
    ...
    def to_gt_vectors(self, gt_dict):
        # import pdb;pdb.set_trace()
        gt_labels = gt_dict['gt_vectorlabels_3d'].data
        gt_instances = gt_dict['gt_vectorbboxes_3d'].data.instance_list
