...
from mmdet3d.bevformer.apis.test import custom_multi_gpu_test
...
data_path_prefix = '/dahuafs/userdata/20704/MapTR-maptrv2'  # project root
...
def draw_visible_polyline_cv2(line, valid_pts_bool, image, color, thickness_px, map_class):
    ...
    for i in range(len(line) - 1):
        ...
        if map_class != 'centerline':
            image = cv2.line(image, pt1=(x1, y1), pt2=(x2, y2), color=color, thickness=thickness_px,
...
def main():
    ...
    for i, data in enumerate(data_loader):
        if ~(data['gt_vectorlabels_3d'].data[0][0] != -1).any():
            # import pdb;pdb.set_trace()
        ...
        gt_bboxes_3d = data['gt_vectorbboxes_3d'].data[0]
        gt_labels_3d = data['gt_vectorlabels_3d'].data[0]

        pts_filename = img_metas[0]['lidar_path']
        ...
        filename_list = img_metas[0]['img_filename']
        img_path_dict = {}
        # save cam img for sample
        # import ipdb;ipdb.set_trace()
        for filepath, lidar2img, img_aug in zip(filename_list, img_metas[0]['lidar2img'],
                                                img_metas[0]['img_aug_matrix']):
        ...
        for pts in gt_dict['centerline']:
            ...
            plt.quiver(x[:-1], y[:-1], x[1:] - x[:-1], y[1:] - y[:-1], scale_units='xy', angles='xy', scale=1,
                       color='green', headwidth=5, headlength=6, width=0.006, alpha=0.8, zorder=-1)
        ...
        # visualize pred
        scores_3d = result_dict['vector_scores_3d']
        labels_3d = result_dict['vector_labels_3d']
        pts_3d = result_dict['vector_pts_3d']
        keep = scores_3d > 0.3
